...
object Main{
    ...
    val url = "jdbc:mysql://localhost:3306/todo" // Modify for whatever port you are running your DB on
    val username = "root"
...
// adds a new task to sql database, uses connection and scanner to receive data to be sent to sql.
  ...
  def addtask(connection:Connection, scanner:Scanner, log:PrintWriter): Unit = {
      ...
      var task_date = scanner.nextInt()
      scanner.nextLine()

      val resultSet = statement.executeUpdate(s"INSERT INTO scala_tasks(task_desc, task_date) VALUES ($task_desc, $task_date);")
      log.write(Calendar.getInstance().getTimeInMillis + " - Executing INSERT INTO scala_tasks(task_desc, task_date) VALUES ($task_desc, $task_date); \n")
 // Change query to your table
  ...
  def viewTask(connection:Connection, log:PrintWriter): Unit = {
    ...
    while ( resultSet.next() ) {
        ...
        println()
        log.write(Calendar.getInstance().getTimeInMillis + " - Executing SELECT * FROM scala_tasks; \n")
      }
  }


  //allows user to edit the task description of a row, chosen by task ID
  def editTask(connection:Connection, scanner:Scanner, log:PrintWriter): Unit = {
    val statement = connection.createStatement()
    ...
    var task_desc = scanner.nextLine()
    task_desc = "'" + task_desc + "'"
    val resultSet = statement.executeUpdate(s"UPDATE scala_tasks SET task_desc = $task_desc  WHERE task_id = $editTask_id; ")
    
    log.write(Calendar.getInstance().getTimeInMillis + " - Executing UPDATE scala_tasks SET task_desc = $task_desc  WHERE task_id = $editTask_id; \n")
  }

  //allows user to delete the task description of a row, chosen by taskID
  def deleteTask(connection:Connection, scanner:Scanner, log:PrintWriter): Unit = {
    val statement = connection.createStatement()
    println("Which task would you like to delete? (Choose by TASK ID)")
    var deleteTask_id = scanner.nextInt()
    scanner.nextLine()
    val resultSet = statement.executeUpdate(s"DELETE FROM scala_tasks WHERE task_id = $deleteTask_id; ")
    log.write(Calendar.getInstance().getTimeInMillis + " - Executing DELETE FROM scala_tasks WHERE task_id = $deleteTask_id; \n")
  }
